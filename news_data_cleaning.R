...
data_2019 <- read_csv("datasets/unc/news_2019.csv")
...
data_2019 <- data_2019 %>% select(-c(avg_free_circulation,avg_paid_circulation))


#filter out empty rows
data_2019 <- data_2019 %>% filter_all(any_vars(complete.cases(.)))


## identify rows with missing values other than in owner_type
data_2019 %>% select(-owner_type) %>% filter_all(any_vars(is.na(.))) %>% View()         
...
data_2019 <- 
  ...
  mutate(
    # rename no-county Virginia counties by appending " city"
    county = if_else(
      is.na(county), paste(data_2019$city," City"),county, missing = data_2019$county),
    city = if_else(
      # fill in missing cities for DC
      county == "District of Columbia", "District of Columbia", city)
  )
...
#make circulation numeric
data_2019 <- data_2019 %>% mutate(total_circulation = as.numeric(gsub(",","",total_circulation)))

#data_2019 %>% summarise(n=n_distinct(total_circulation), na = sum(is.na(total_circulation)), med = median(total_circulation, na.rm=TRUE))
#data_2019$total_circulation %>% str()

# replace NA in total_circulation with average
#data_2019 %>% mutate(total_circulation = replace(
#                                                  total_circulation, is.na(total_circulation),median(total_circulation,na.rm=TRUE)
#)) %>% select(-owner_type) %>% filter_all(any_vars(is.na(.))) %>% View()

data_2019 <- data_2019 %>% mutate(total_circulation = replace(
  total_circulation, is.na(total_circulation),median(total_circulation,na.rm=TRUE)
))
...
data_2019 %>% group_by(days_published) %>% summarise(count=n())

data_2019 <- data_2019 %>% mutate(days_published = replace(
  days_published, is.na(days_published), median(days_published, na.rm=TRUE)                    
))


data_2019 %>% filter_all(any_vars(is.na(.))) %>% View()


# fill missing owner_types (see bottom for more detailed clean-up)

data_2019 %>% group_by(owner_type) %>% summarise(n=n())

data_2019 <- data_2019 %>% mutate(owner_type = replace(owner_type, is.na(owner_type),"unknown"))

data_2019_new <- data_2019


write.csv(data_2019_new, "data_2019_cleaned.csv")
